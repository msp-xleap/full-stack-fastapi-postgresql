...
class AgentGenerationLock:
    ...
    def release(self):
        ...
        if self._lock is not None:
            agent_manager.release_generation_lock(
                lock=self._lock, agent_id=self.agent_id, last_id=self.last_id
            )
...
class AgentContributionLock:
    ...
    def release(self):
        ...
        if self.lock is not None:
            agent_manager.release_contribution_lock(
                lock=self.lock, agent_id=self.agent_id
            )
...
class AgentManager:
    ...
    _generation_context: dict[uuid_pkg.uuid4, uuid_pkg.uuid4] = {}
    ...
    def _find_generation_lock(
        self, agent_id: uuid_pkg.uuid4
    ) -> threading.Lock:
    ...
    def acquire_contribution_lock(
        self, agent_id: uuid_pkg.uuid4
    ) -> AgentContributionLock:
        ...
        logging.info(
            f"AgentManager.lock acquiring contribution for agent: {agent_id} ..."
        )
        lock.acquire(blocking=True)
        logging.info(
            f"AgentManager.lock SUCCESSFULLY acquired contribution for agent: {agent_id}"
        )
    ...
    def try_acquire_generation_lock(
        ...
        if lock.acquire(blocking=False):
            logging.info(
                f"AgentManager.lock SUCCESSFULLY acquired generation lock for agent: {agent_id}"
            )
            return AgentGenerationLock(
                acquired=True, lock=lock, agent_id=agent_id, last_id=last_uuid
            )
        else:
            logging.info(
                f"AgentManager.lock UNSUCCESSFULLY generation lock not acquired for agent: {agent_id}"
            )
            return AgentGenerationLock(
                acquired=False, agent_id=agent_id, last_id=last_uuid
            )

    def release_contribution_lock(
        self, agent_id: uuid_pkg.uuid4, lock: threading.Lock
    ):
        """Only to be called from AgentLock.release()"""
        if (
            agent_id in self._contribution_lock
            and lock == self._contribution_lock[agent_id]
        ):
            lock.release()
            logging.info(
                f"AgentManager.release contribution lock was released: {agent_id}"
            )
        else:
            logging.info(
                f"AgentManager.release contribution  lock was not released: {agent_id}"
            )
    ...
    def release_generation_lock(
        ...
        agent_id: uuid_pkg.uuid4,
        lock: threading.Lock,
        last_id: uuid_pkg.uuid4 = None,
    ):
        """Only to be called from AgentLock.release()"""
        if (
            agent_id in self._generation_locks
            ...
            lock.release()
            logging.info(
                f"AgentManager.release generation lock was released: {agent_id}, {self._last_generation[agent_id]}"
            )
        else:
            logging.info(
                f"AgentManager.release generation  lock was not released: {agent_id}"
            )
