...
def create_ai_agent_briefing2_reference(
    *,
    ...
    langfuse_ref_base = langfuse_base_from_briefing_reference_base(
        session, briefing_ref_base
    )
...
def get_ai_agent_references(
    *, session: Session, agent: AIAgent
) -> list[Briefing2Reference]:
    statement = select(Briefing2Reference).where(
        Briefing2Reference.agent_id == agent.id
    )
    session_user = session.exec(statement).all()
    return list(session_user)


def get_ai_agent_file_references(
    *, session: Session, agent: AIAgent
) -> list[Briefing2Reference]:
    statement = (
        select(Briefing2Reference)
...
def create_or_update_ai_agent_briefing2(
    *,
    ...
    try:
        ...
        langfuse_base = langfuse_base_from_briefing_base(
            session, briefing_base
        )
        ...
        briefing = create_ai_agent_briefing2(
            session=session, ai_agent=ai_agent, briefing_base=briefing_base
        )
        return briefing
...
def replace_briefing2_references(
    *,
    session: Session,
    agent_id: str,
    briefing_refs: list[AIBriefing2ReferenceBase],
) -> None:
    ...
    new_map = {
        briefing_ref_base.ref_id: briefing_ref_base
        for briefing_ref_base in briefing_refs
    }
